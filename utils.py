...
def visualize_game(env, agent, path='images/game_visual.png', debug=False,
                    animate=False, fps=10, minimum_score = 10, looping_length = 1000):
    game_images = []
    qvalues = []
    food_count = []
    ...
    if food_count[-1] < minimum_score:
        print(f'Score below minimum score of {minimum_score}, not rendering game')
    elif len(set(food_count[-looping_length:])) == 1:
        print(f'Game is looping, not rendering game')
    else:
        ...
        # plot the game
        if(animation):
            print("Start of create animation")
